...
var (
	...
	judgerMutex      map[int64]*sync2.Mutex

	taskQueue chan *CSubmission
	taskGroup map[int64]chan *CSubmission
)
...
func contestInit() {
	...
	judgerMutex = make(map[int64]*sync2.Mutex)
	taskQueue = make(chan *CSubmission, 1000000)
	taskGroup = make(map[int64]chan *CSubmission)
	go taskDistribution()
}
...
func ContestJudger(cs *CSubmission) {
	...
	if orgScore := mp["score"]; orgScore < 100 {
		if newScore == 100 {
			team.Scores += 100 - orgScore
			team.Solved++
			mp["score"] = 100
			mp["minutes"] = uint(cs.CreatedAt.Sub(OneCol(cd, "begin").ToTime()).Minutes())
			team.Penalty += mp["fail_times"]*20 + mp["minutes"]
			cp.AC++
			cp.All++
			if cp.FirstSolveTime == 0 || cp.FirstSolveTime > mp["minutes"] {
				cp.FirstSolveTime = mp["minutes"]
			}
		} else {
			cp.All++
			mp["fail_times"]++
...
func taskDistribution() {
	...
	for {
		cs := <-taskQueue
		judgeID := cs.JudgeID()
		if _, ok := judgerMutex[judgeID]; !ok {
			judgerMutex[judgeID] = new(sync2.Mutex)
		}
		mut = judgerMutex[judgeID]
		mut.Lock()
		if _, ok := taskGroup[judgeID]; !ok {
			mut.Unlock()
			taskGroup[judgeID] = make(chan *CSubmission, 1000)
			taskGroup[judgeID] <- cs
			go taskHandle(judgeID)
		} else {
			taskGroup[judgeID] <- cs
			mut.Unlock()
		}
